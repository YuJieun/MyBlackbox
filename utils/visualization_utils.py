...
import webcolors
from .find_color import ColorNames
...
# print('second')
# print(SocketInfo.ADDR)
# csock.connect_ex(SocketInfo.ADDR)
# print('third')
...
def visualize_boxes_and_labels_on_image_array(
    ...
    # person,bicycle,car,motorcycle,bus,cat,dog,cellphone만 나오게 해줌 --- 지은추가

    # 아니면 위의 skip_labels을 활용하자   --->11/1추가
    n = len(classes)
    idxs = []
    for i in range(n):
        if classes[i] in [1, 2, 3, 4, 6, 14, 17, 18, 77]:
            idxs.append(i)
    boxes = boxes[idxs, ...]
    scores = scores[idxs, ...]
    ...
    for box, detect_info in box_to_color_map.items():
        ...
        print(box_to_display_str_map[box], " , ", box, detect_info[0],
              detect_info[2])  # label, scores, y1,x1,y2,x2순으로 출력!
...
def detect_color(image, box, name):
    image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
    im_width, im_height = image_pil.size

    ymin, xmin, ymax, xmax = box
    wvalue = 0.2 * (xmax - xmin)
    hvalue = 0.2 * (ymax - ymin)
    (left, right, top, bottom) = (
    (xmin + wvalue) * im_width, (xmax - wvalue) * im_width, (ymin + hvalue) * im_height, (ymax - hvalue) * im_height)

    img_trim = image[int(top):int(bottom), int(left):int(right)]
    # print(img_trim)
    image_trim_pil = Image.fromarray(img_trim, 'RGB')

    image_trim_pil = image_trim_pil.resize((1, 1))
    color_bgr = image_trim_pil.getpixel((0, 0))
    if name == 'car':
        color_name = ColorNames.findNearestWebColorName(color_bgr)
    else:
        color_name = 'None'

    return color_bgr, color_name


def detect_location(image, box, name):
    image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
    im_width, im_height = image_pil.size

    ymin, xmin, ymax, xmax = box
    locate = [(xmin + xmax) / 2 * im_width, ymax * im_height]
    # print("locate: ", locate)

    title = None
    if name == 'car':
        if locate[0] > 200 and locate[0] < 1700 and locate[1] > 900 and locate[1] < 1050:
            # img_trim = image[int(top):int(bottom), int(left):int(right)]
            xcond = (xmax - xmin) / 8
            ycond = (ymax - ymin) / 5
            trim = ((xmin + xcond) * im_width, (ymin + ycond) * im_height, (xmax - xcond) * im_width,
                    (ymax - ycond) * im_height)
            crop = image_pil.crop(trim)
            print("------------------------------crop---------------------------------")
            title = 'C:/tf_object_detection_api/object_detection/crops/' + time.strftime('%y%m%d_%H%M%S') + '.png'
            crop.save(title)

    if locate[0] > 800 and locate[0] < 900:
        return "front", title
    elif locate[0] <= 800:
        if locate[1] < 800:
            return "leftfront", title
        elif locate[1] >= 800:
            return "left", title
    elif locate[0] >= 900:
        if locate[1] < 800:
            return "rigntfront", title
        elif locate[1] >= 800:
            return "right", title
    else:
        return "else", title